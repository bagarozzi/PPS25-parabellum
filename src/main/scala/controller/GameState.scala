 ...
 */
class GameState(val manager: TurnManager,val obstacles: Set[Obstacle], val projectiles: Option[Projectile], val pendingFunction: Option[String])
object GameState:
  ...
  def addObstacle(g: GameState, obstacle: Obstacle): GameState =
    GameState(g.manager, g.obstacles + obstacle, g.projectiles, None)
