...
| 07 | Interfaccia grafica di base |  | . | no |
| 08 | Architettura di base: flusso di dati tra model, view, viewmodel |  | . | no |
| 09 | Inserimento (tramite ViewModel e Engine) e parsing dei parametri di una retta |  | . | no |
| 10 | Creazione dell'Engine per stato del gioco e game loop |  | . | no |
| 11 | Gestione delle collisioni dei proiettili |  | . | no |
